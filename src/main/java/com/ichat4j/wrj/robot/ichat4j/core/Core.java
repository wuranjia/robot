package com.ichat4j.wrj.robot.ichat4j.core;
...
public class Core {
    ...
    private List<String> filterList = new ArrayList<String>();//过滤集合
    ...
    public boolean match(String fromUserName) {
        for (JSONObject jsonObject : this.getContactList()) {
            String nickName = jsonObject.getString("NickName");
            String userName = jsonObject.getString("UserName");
            if (StringUtils.isNotBlank(fromUserName)
                    && fromUserName.equals(userName)) {
                if (StringUtils.isNotBlank(nickName)
                        && filterList.contains(nickName)) {
                    return true;
                }
            }
        }
